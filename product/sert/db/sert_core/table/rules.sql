...
--changeset mipotter:create_table_sert_core.rules endDelimiter:; runOnChange:true runAlways:false rollbackEndDelimiter:;
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tables where owner = upper('${sert_core_schema}') and table_name = 'RULES';
CREATE TABLE  ${sert_core_schema}.rules (
    rule_id               NUMBER
...
ALTER TABLE  ${sert_core_schema}.rules ADD CONSTRAINT rules_pk PRIMARY KEY ( rule_id );

ALTER TABLE  ${sert_core_schema}.rules ADD CONSTRAINT rules_un UNIQUE ( rule_key,
                                                   apex_version );
--rollback drop table ${sert_core_schema}.rules;
