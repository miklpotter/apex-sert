...
--changeset mipotter:create_table_sert_core.comments endDelimiter:; runOnChange:true runAlways:false rollbackEndDelimiter:;
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tables  where owner = upper('${sert_core_schema}') and table_name = 'COMMENTS';
CREATE TABLE  ${sert_core_schema}.comments (
    comment_id       NUMBER
...
LOGGING;
 ...
 ALTER TABLE ${sert_core_schema}.comments ADD CONSTRAINT comments_pk PRIMARY KEY ( comment_id );
--rollback drop table ${sert_core_schema}.comments;